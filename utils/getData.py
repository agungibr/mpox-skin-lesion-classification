...
class getImageLabel(Dataset):
    def __init__ (self, 
        ...
        for fold in folds:
            ori_train = (original + f"fold{fold}/Train")
            for i, pox in enumerate(sorted(listdir(ori_train))):
                for image_name in listdir(ori_train + "/" + pox):
                    image = cv.resize(cv.imread(ori_train + "/" + pox + "/" + image_name), (32, 32)) / 255
                    self.dataset.append([image, to_one_hot[i]])
            
            aug = (augmented + f"fold{fold}_AUG/Train")
            ...
            ori_test = (original + f"fold{fold}/Test")
            for i, pox in enumerate(sorted(listdir(ori_test))):
                for image_name in listdir(ori_test + "/" + pox):
                    image = cv.resize(cv.imread(ori_test + "/" + pox + "/" + image_name), (32, 32)) / 255
                    self.dataset.append([image, to_one_hot[i]])

            ori_valid = (original + f"fold{fold}/Valid")
            for i, pox in enumerate(sorted(listdir(ori_valid))):
                for image_name in listdir(ori_valid + "/" + pox):
                    image = cv.resize(cv.imread(ori_valid + "/" + pox + "/" + image_name), (32, 32)) / 255
                    self.dataset.append([image, to_one_hot[i]])
            
    def __getitem__(self, item):
        feature, label = self.dataset[item]
        #return torch.tensor(feature, dtype=torch.float32), torch.tensor(label, dtype=torch.float32)
        label_index = torch.argmax(torch.tensor(label, dtype=torch.float32))  # Convert one-hot to class index
